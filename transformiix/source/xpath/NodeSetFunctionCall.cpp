 ...
 *    
 * $Id: NodeSetFunctionCall.cpp,v 1.1 2001/01/07 05:41:39 jdike Exp $
 */
...
ExprResult* NodeSetFunctionCall::evaluate(Node* context, ContextState* cs) {
    ...
    switch ( type ) {
	    ...
	    if ( nodeSet ){
	        int i;
		int position = 0;
		for (i=0;i<nodeSet->indexOf(context)+1;i++) {
		    Node* aNode = nodeSet->get(i);
		    if (aNode->getNodeType() == context->getNodeType())
		        position++;
		}
		result = new NumberResult(position);
	    }
            else
                result = new NumberResult(0.0);
            break;
