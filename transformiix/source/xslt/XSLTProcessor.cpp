 ...
 *
 * $Id: XSLTProcessor.cpp,v 1.1 2001/01/07 05:41:39 jdike Exp $
 */
    ...
    (Document& document, OutputFormat* format, ostream& out)
{
    ...
    xmlPrinter->setOmitXMLDeclaration(format->getOmitXMLDeclaration());
        ...
        if (node->getNodeType() == Node::ELEMENT_NODE) {
            ...
            switch (getElementType(name, ps)) {
                ...
                case XSLType::OUTPUT :
                {
                    ...
                    attValue = element->getAttribute(OMIT_XMLDECL_ATTR);
                    if (attValue.length() > 0){
		        MBool omitDeclaration = attValue.isEqual(YES_VALUE);
    ...
    //-- handle element nodes
        ...
        switch ( getElementType(nodeName, ps) ) {
            ...
            case XSLType::TEXT :
            {
                String data;
                String disableEsc = 
		    actionElement->getAttribute(DISABLE_OUT_ESC);
                //-- get Node value, and do not perform whitespace stripping
                XMLDOMUtils::getNodeValue(actionElement, &data);
		disableEsc.toLowerCase();
		if(disableEsc.isEqual("no")){
		    const String temp = data;
                    data.clear();
		    XMLUtils::replaceXMLEntities(temp, data);
		}
		...
		if(disableEsc.isEqual("no")){
		    const String temp = value;
                    value.clear();
		    XMLUtils::replaceXMLEntities(temp, value);
		}
