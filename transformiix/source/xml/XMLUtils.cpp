 ...
 *    -- unicode bug fix
 *
 * $Id: XMLUtils.cpp,v 1.1 2001/01/07 05:41:38 jdike Exp $
 */
/**
...
void XMLUtils::replaceXMLEntities(const String& data, String& dest) {
    unsigned token;
    Int32 oldSize = data.length();
    unsigned* chars;
    unsigned* oldChars = new unsigned[oldSize+1];



    data.toUnsignedArray(oldChars);
    ...
    Int32 newSize = 0;
        ...
        token = oldChars[i];
        switch(token) {
            case '\"': newSize+=6; break;
            case '<':
            case '>': newSize+=4; break;
            case '&': newSize+=5; break;
